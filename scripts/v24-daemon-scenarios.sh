...
artifact_dir="$root/artifacts/generated/v24-daemon-scenarios"
manifest_file="$artifact_dir/manifest.txt"
run_log="$artifact_dir/run.log"
podman_image="docker.io/library/golang:1.24.12"
scenarios_file="$root/containers/v2.4/scenarios.conf"
...
{
    echo "suite: v24-daemon-scenarios"
...
for result in "${scenario_records[@]}"; do
    ...
    echo "[v24-daemon-scenarios] $result_slug -> $result_status"
...
if [[ "$overall_fail" == "true" ]]; then
    echo "[v24-daemon-scenarios] one or more scenario probes failed"
    exit 1
fi

echo "[v24-daemon-scenarios] wrote manifest: $manifest_file"
echo "[v24-daemon-scenarios] wrote log: $run_log"
exit 0
