...
artifact_dir="$root/artifacts/generated/v23-regression-scenarios"
manifest_file="$artifact_dir/manifest.txt"
run_log="$artifact_dir/run.log"
podman_image="docker.io/library/golang:1.24.12"
scenarios_file="$root/containers/v2.3/scenarios.conf"
...
run_probe() {
    ...
    if [[ "$podman_available" != "true" ]]; then
        ...
        podman run --rm --userns=keep-id \
          ...
          -w /workspace \
          "$podman_image" \
          bash -lc "cd /workspace && /usr/local/go/bin/go test $pkg -run $pattern -count=1 -v" \
          >"$log_path" 2>&1
        actual_exit=$?
        set -e
...
{
    echo "suite: v23-regression-scenarios"
...
for result in "${scenario_records[@]}"; do
    ...
    echo "[v23-regression-scenarios] $result_slug -> $result_status"
...
if [[ "$overall_fail" == "true" ]]; then
    echo "[v23-regression-scenarios] one or more scenario probes failed"
    exit 1
fi

echo "[v23-regression-scenarios] wrote manifest: $manifest_file"
echo "[v23-regression-scenarios] wrote log: $run_log"
exit 0
