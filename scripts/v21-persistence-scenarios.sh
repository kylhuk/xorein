...
artifact_dir="$root/artifacts/generated/v21-persistence-scenarios"
...
run_probe() {
  ...
  scenario_records+=("$label|$status|$actual_exit|$reason|$pkg|$pattern|$required_output|$log_path|$command_label")
...
run_probe "multi-peer-restart-resume" "multi-peer restart/resume probe" "./pkg/v21/store/tombstone" "TestRestartInvariant"
run_probe "local-history-clear-recovery" "local history clear/recovery probe" "./pkg/v21/store/retention" "TestPruneWindowAndLimit"
run_probe "relay-no-long-history-hosting" "relay no-long-history-hosting boundary probe" "./pkg/v21/search" "TestSearchFiltersAndCoverage"

{
  echo "suite: v21-persistence-scenarios"
  ...
  for result in "${scenario_records[@]}"; do
    IFS='|' read -r result_name result_status result_exit result_reason result_pkg result_pattern result_required result_log result_cmd <<<"$result"
...
has_failures="false"
for result in "${scenario_records[@]}"; do
  IFS='|' read -r result_name result_status _ <<<"$result"
  echo "[v21-persistence-scenarios] $result_name -> $result_status"
...
if [[ "$has_failures" == "true" ]]; then
  echo "[v21-persistence-scenarios] one or more scenario probes failed"
  exit 1
fi

echo "[v21-persistence-scenarios] wrote manifest: $manifest_file"
echo "[v21-persistence-scenarios] wrote log: $run_log"
exit 0
