...
artifact_dir="$root/artifacts/generated/v25-blob-scenarios"
manifest_file="$artifact_dir/manifest.txt"
run_log="$artifact_dir/run.log"
podman_image="docker.io/library/golang:1.24.12"
scenarios_file="$root/containers/v2.5/scenarios.conf"
...
for result in "${scenario_records[@]}"; do
    ...
    echo "[v25-blob-scenarios] $result_slug -> $result_status"
...
if [[ "$overall_fail" == "true" ]]; then
    echo "[v25-blob-scenarios] one or more scenario probes failed"
    exit 1
fi

echo "[v25-blob-scenarios] wrote manifest: $manifest_file"
echo "[v25-blob-scenarios] wrote log: $run_log"
exit 0
